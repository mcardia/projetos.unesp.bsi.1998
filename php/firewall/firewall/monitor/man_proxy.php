...
<?
	...
	switch ( $erro )
	{
		case 1 : show_message("Este registro já está cadastrado"   , "frame.php?link=proxy"); break;
		case 2 : show_message("Não foi possível $acao o registro"  , "frame.php?link=proxy"); break;
		default: show_message("Operação realizada com sucesso"  , "frame.php?link=proxy"); break;
	}
...
<?
	function incluir()
	{
		...
		$str_sql = "SELECT id FROM proxy WHERE host = '$host' OR servidor='$servidor'";
		...
		if ( ! $erro )
		{
			$str_sql  = "INSERT INTO proxy (servidor, host, port, id_status) ";
...
<?
	function excluir()
	{
		...
		$str_sql = "DELETE FROM proxy WHERE id=$id_server";
		$rs = $db->db_query($str_sql);
